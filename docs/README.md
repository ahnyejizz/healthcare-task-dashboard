...
> 현재 배포 버전의 `할 일 삭제` 기능은 "화면 시연용 mock 동작"입니다.
> 로컬 개발 환경에서는 삭제 후 목록이 바로 갱신되지만,
> Vercel 배포 환경에서는 삭제 결과가 계속 유지되지 않을 수 있습니다.

- 이유: 할 일 데이터가 DB가 아니라 서버 프로세스 메모리의 mock 저장소에만 저장됨
- 로컬: 개발 서버 프로세스가 계속 살아 있어 삭제한 결과가 바로 다음 조회에도 반영됨
- 배포: 서버리스 환경 특성상 요청마다 다른 인스턴스가 응답하거나 메모리가 초기화될 수 있어, 삭제 후 다시 목록에 보일 수 있음
- 결론: 배포 환경에서 삭제 결과를 안정적으로 유지하려면 외부 저장소 연결이 필요함
...
- 배포 환경에서는 삭제 요청이 성공해도, mock 메모리 저장소 특성상 목록 재조회 시 삭제 결과가 유지되지 않을 수 있음
...
- 로그아웃 시에는 클라이언트에 저장된 `accessToken`을 제거하고, 서버에서 `refreshToken` 쿠키를 만료 처리
...
- 태스크 ID 오름차순, 내림차순에 따른 **`정렬` 기능** 제공
- 태스크 ID, 태스크명, 메모 key에 따른 **`검색` 기능** 제공
- 검색/필터 결과가 없는 경우 아이콘이 포함된 **공통 `EmptyState` UI**로 우측 콘텐츠 영역 전체를 활용해 엠티셋 안내 제공
- 상세/목록 화면 **view 모드를 `카드형, 리스트형` 형태**로 볼 수 있도록 제공

[회원정보]
- `UserResponse`에는 `name`, `memo`만 포함되므로, `email`은 세션 정보(`SessionResponse`)에서 별도 조회해 화면에서 함께 제공

[공통]
- **Prettier 설정** 추가
- **로그아웃 기능** 추가 및 **로그아웃 확인 알럿창** 추가
- API 로딩 시 **전체화면 오버레이 로딩바** 추가 및 공통화
- 페이지별 **동일 여백 템플릿** 추가 및 공통 적용
- **컴포넌트 분리** : 필터링/정렬/검색/뷰모드 제어 UI 분리
- 파일명만 봐도 메인 페이지 역할을 알 수 있도록 **파일명 명명 규칙 정의** (-page.tsx)
- **`desc` 주석 관리를 위한 문서** 추가 및 규칙 적용 (`docs/comment-rules.md`)
- **`import` 정렬 및 레이어 주석 관리를 위한 문서** 추가 및 규칙 적용 (`docs/import-rules.md`)
- **다양한 해상도에 따른 반응형 레이아웃** 처리
